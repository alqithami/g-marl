#!/usr/bin/env python3
"""
G‑MARL: Graph‑based Multi‑Agent Reinforcement‑Learning reference pipeline
-----------------------------------------------------------------------
• End‑to‑end training / evaluation script ready for **RunPod** or local GPU.
• PyTorch 2.3  +  PyTorch‑Geometric 2.5  +  rich CLI via argparse / yaml.
• Modular dataset loader (Intel‑Lab, SWaT, WADI, TON‑IoT, synthetic).
• PPO with GAE, mini‑batch optimisation, checkpointing & TensorBoard.
• Energy‑aware reward, selective communication gate, GAT encoder.

Author : OpenAI ChatGPT reference implementation  –  2025‑05‑27
License: Apache‑2.0
"""
...
class SyntheticIoTGraphDataset(torch.utils.data.Dataset):
    """Generate synthetic graph‑stream episodes on‑the‑fly."""
    ...
    def __init__(self,
        ...
        # Pre‑generate static topology once per episode
    ...
    # .................................................................
    def __len__(self):
        return self.T - 1  # each step predicts next‑state anomaly
...
# TODO: RealDatasetLoader class for Intel‑Lab, SWaT, WADI, TON‑IoT.
...
def main():
    ...
    for ep in range(cfg["train"]["episodes"]):
        ...
        for t, (data, labels) in enumerate(loader):
            ...
            logp = dist_det.log_prob(a_det) + dist_comm.log_prob(a_comm)
            # simple reward: +1 correct, −2 miss, −0.5 false+, comm penalty
